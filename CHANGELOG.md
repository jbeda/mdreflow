...
- A paragraph containing a link wrapped across a line break now reflows instead of being skipped forever (#18, contributed by Karl Isenberg): the spanning-bracket guard arms only when a `]:` — the one shape a link reference definition can form from — appears in the paragraph. Being skipped was self-perpetuating: the skip preserved the very line break that caused it, stranding 60 lines on the reporter's docset.
